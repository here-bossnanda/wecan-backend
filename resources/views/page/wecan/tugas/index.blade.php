...
<div class="page-title-box">
  <div class="container-fluid">
    <div class="row align-items-center">
      <div class="col-md-8">
        <h4 class="page-title mb-1">Tugas Wecan</h4>
        <ol class="breadcrumb m-0">
          <li class="breadcrumb-item"><a href="{{url('/')}}">Dashboard</a></li>
          <li class="breadcrumb-item active">Tugas Wecan</li>
        </ol>
      </div>
      {{-- <div class="col-md-4">
      ...
      <div class="col-xl-12">
        <div class="card">
          <div class="card-header">
            @if (Auth::user()->can('tugas.create'))
              <div class="float-left">
              <button onclick="addForm()" type="button" class="btn btn-primary btn-block"><i class="fa fa-plus-square"> Tambah Tugas</i> </button>
            </div>
            @endif
          ...
          <div class="card-body">
            <h4 class="header-title">Tugas Wecan</h4>
            <table class="table table-bordered dt-responsive table-striped nowrap table-tugas-wecan" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
              <thead>
                <tr>
                  <th>No</th>
                  <th>Nama</th>
...
@include('page.wecan.tugas.modal')
@endsection
@push('scripts')
<script type="text/javascript">
  ...
  $(function(){
    table = $('.table-tugas-wecan').DataTable({
        "language": {
      ...
      "serverSide" : true,
      "ajax":{
        "url" : "{{route('tugas-wecan.data')}}",
        "type" : "GET"
      }
    });
    $('#modal-tugas-wecan form').validator().on('submit', function(e){
      if(!e.isDefaultPrevented()){
        var id = $('#tugas_id').val();
        if (save_method == "add")
        url = "{!! route('tugas-wecan.store') !!}";
        else url = "tugas-wecan/"+id;
        $.ajax({
          url : url,
          type : "POST",
    ...
    $("select#aktivasi_wecan_id").select2({
      ...
      dropdownParent: $("#modal-tugas-wecan"),
      placeholder: 'Pilih Wecan',
      minimumInputLength: 1,
      ajax: {
        url: '{{route('tugas-wecan.select2wecan')}}',
        dataType: 'json',
  ...
  function addForm(){
    ...
    $('#modal-tugas-wecan').modal('show');
    $('#modal-tugas-wecan form')[0].reset();
        ...
        $.ajax({
          ...
          success: function(data){
            if(data.status == "failed"){
              swal("Error deleting!", "tugas wecan tidak bisa dihapus!","warning");
              toastr.warning('tugas wecan sedang digunakan tidak bisa dihapus!', 'Warning Alert', {timeOut: 4000});
            }else{
  ...
  function tugasWecan(id){
    swal({
      ...
      if (result.value) {
        $.ajax({
          url: 'tugas-wecan/' + id + '/aktivasi',
          type: 'POST',
          data: {
            '_token': $('input[name=_token]').val(),
          },
          success: function(data){
            swal("Done!", "tugas wecan Berhasil diubah!", "success");
            toastr.success('tugas wecan Berhasil diubah!', 'Success Alert', {timeOut: 4000});
            table.ajax.reload();
          },
          error : function(data){
